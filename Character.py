...
class Character(pygame.sprite.Sprite):
    ...
    def controller(self, keys, collisions = []):
        ...
        if not self.in_battle:
           ...
           if keys[pygame.K_SPACE] and self.character_class == 'Warrior' and not self.is_hit:
               self.attacking = True
               self.idling = True
    ...
    def draw(self, window, adjusted_rect = pygame.Rect(0, 0, 1280, 720)):
        ...
        if self.is_hit:  # hit animation if hit is true and idle is true
        ...
        elif not self.idling and not self.attacking and not self.is_hit:
            ...
            if self.attacking:#attack animation if attack is true and idle is true
    ...
    def draw_ui(self, window, profile_frame, health_bar_frame, action_points_frame, offset):
        ...
        self.health_bar = pygame.transform.scale(self.health_bar, (int(self.health_bar_width*health_percentage), self.health_bar.get_height()))
        self.action_points_bar = pygame.transform.scale(self.action_points_bar, (self.action_points_bar.get_width() ,int(self.action_points_bar_height*action_points_percentage)))
